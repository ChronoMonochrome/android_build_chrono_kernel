...
package-full: clean modules-install package-modules package-ramdisk
	...
	rm -f $(KERNEL_NAME);
	zip -9r $(KERNEL_NAME) META-INF system genfstab ramdisk boot.img ramdisk.7z 7za scripts init.d
...
package-full-selinux: clean modules-install-selinux package-modules package-ramdisk
	...
	zip -9r $(KERNEL_NAME_SELINUX) system META-INF genfstab ramdisk boot.img ramdisk.7z 7za scripts init.d
...
package-full-cm13: clean modules-install-cm13 package-modules package-ramdisk
	...
	zip -9r $(KERNEL_NAME_CM13) META-INF system genfstab ramdisk boot.img ramdisk.7z 7za scripts init.d
...
package-full-nodebug: clean modules-install-nodebug package-modules package-ramdisk
	...
	rm -f $(KERNEL_NAME);
	zip -9r $(KERNEL_NAME_NODEBUG) META-INF genfstab ramdisk boot.img ramdisk.7z 7za scripts init.d
...
package-private: clean modules-install-nodebug package-modules
	...
	zip -9r $(KERNEL_NAME_PRIVATE) META-INF system ramdisk boot.img scripts/update_modules.sh scripts/main.sh \
		scripts/remote_modules.sh scripts/check_ramdisk_partition.sh scripts/initd_install.sh init.d
